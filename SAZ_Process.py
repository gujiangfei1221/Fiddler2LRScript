...
class SAZ_Process(object):
    ...
    metadata_regexp2 = re.compile("raw/(?P<id>[0-9]+)_m\.xml")
    request_pattern = "raw/{}_c.txt"
    request_pattern2 = "raw/{}_m.xml"
    ...
    def GetRequestList2(self, saz_name):
        RequestList = []
        with ZipFile(saz_name, 'r') as saz_file:
            for saz_element in saz_file.namelist():
                self.metadata_match = self.metadata_regexp2.match(saz_element)
                if self.metadata_match is not None:
                    id = self.metadata_match.group("id")
                    request_name = self.request_pattern2.format(id)
                    request_file = saz_file.open(request_name)
                    request_raw = request_file.readlines()
                    request_content = []
                    for i in range(len(request_raw)):  # 去除每行末尾的'\r\n'
    ...
    def ParseRequest2(self, request):
        requestDict = {}
        elementNum = len(request)
        for i in request:
            if('ui-comments' in i):
    ...
    ''' 将RequestList中的每个request转换为dict，最终将所有request以二维dict的形式返回'''
    def ParseRequestList2(self, requestList):
        requestDictList = []
        for i in range(len(requestList)):
            requestDictList.append(self.ParseRequest2(requestList[i]))
        return requestDictList
